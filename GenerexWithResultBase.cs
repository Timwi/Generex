...
{
    /// <summary>Abstract base class for <see cref="Generex{T,TResult}"/> and <see cref="Stringerex{TResult}"/>.</summary>
        ...
        where TGenerexMatch : GenerexMatch<T, TResult>
    {
        ...
        protected TCombinedGenerex thenExpect<TOtherGenerex, TOtherGenerexMatch, TOtherResult, TCombinedGenerex, TCombinedGenerexMatch, TCombinedResult>(
            GenerexWithResultBase<T, TOtherResult, TOtherGenerex, TOtherGenerexMatch> other, Func<TResult, TOtherGenerexMatch, TCombinedResult> selector, Func<TGenerexMatch, Exception> exceptionGenerator)
            where TOtherGenerex : GenerexWithResultBase<T, TOtherResult, TOtherGenerex, TOtherGenerexMatch>
            where TOtherGenerexMatch : GenerexMatch<T, TOtherResult>
            where TCombinedGenerex : GenerexWithResultBase<T, TCombinedResult, TCombinedGenerex, TCombinedGenerexMatch>
            where TCombinedGenerexMatch : GenerexMatch<T, TCombinedResult>
        {
            return thenExpect<TOtherGenerex, LengthAndResult<TOtherResult>, TOtherGenerexMatch, TCombinedGenerex, LengthAndResult<TCombinedResult>, TCombinedGenerexMatch>(
                other, (input, startIndex, match) => exceptionGenerator(createMatch(input, startIndex, match)),
                (input, startIndex, m1, m2) => new LengthAndResult<TCombinedResult>(selector(m1.Result, other.createMatch(input, startIndex, m2)), m1.Length + m2.Length));
        }
        ...
        /// </summary>
        /// <param name="predicate">The predicate to match a single element against.</param>
        /// <param name="selector">A selector that returns the result object for the new regular expression based on the element matched by the <paramref name="predicate"/>.</param>
        ...
        /// <summary>
        /// Returns a regular expression that matches either this regular expression or a single element using the specified equality comparer (cf. <c>|</c> in traditional regular expression syntax).
        /// </summary>
        /// <param name="element">The element to match.</param>
        /// <param name="selector">A selector that returns the result object for the new regular expression based on the <paramref name="element"/> matched.</param>
        /// <param name="comparer">An optional equality comparer to use against the <paramref name="element"/>.</param>
        ...
        /// <summary>
        /// Returns a regular expression that matches either this regular expression or the specified sequence of elements (cf. <c>|</c> in traditional regular expression syntax).
        /// </summary>
        /// <param name="elements">The sequence of elements to match.</param>
        /// <param name="selector">A selector that returns the result object for the new regular expression based on the sequence matched by the <paramref name="elements"/>.</param>
        /// <param name="comparer">An optional equality comparer to use against the elements in <paramref name="elements"/>.</param>
        ...
        /// </remarks>
        public Generex<T, TResult> ThenResult(Func<TGenerexMatch, Generex<T>> selector)
        {
            return then<Generex<T, TResult>, LengthAndResult<TResult>, GenerexMatch<T, TResult>, TGenerexMatch>(m => selector(m).Process(_ => m.Result), createMatch);
        }
        ...
        /// <see cref="GenerexBase{T, TMatch, TGenerex, TGenerexMatch}.Then{TOtherGenerex, TOtherMatch, TOtherGenerexMatch}(Func{TGenerexMatch, GenerexBase{T, TOtherMatch, TOtherGenerex, TOtherGenerexMatch}})"/>.
        /// </remarks>
        public TOtherGenerex ThenRaw<TOtherGenerex, TOtherMatch, TOtherGenerexMatch>(Func<TResult, GenerexBase<T, TOtherMatch, TOtherGenerex, TOtherGenerexMatch>> selector)
            where TOtherGenerex : GenerexBase<T, TOtherMatch, TOtherGenerex, TOtherGenerexMatch>
            where TOtherGenerexMatch : GenerexMatch<T>
        {
            return then<TOtherGenerex, TOtherMatch, TOtherGenerexMatch, TResult>(selector, (input, startIndex, match) => match.Result);
        }

        /// <summary>
        /// Returns a regular expression that matches this regular expression, then uses a specified <paramref name="selector"/>
        /// to create a new regular expression from the result of the match, then matches the new regular expression
        ...
        /// <summary>Returns a regular expression that matches the specified regular expression (first operand) followed by a single element that satisfies the specified predicate (second operand).</summary>
        ...
        /// <summary>Returns a regular expression that matches a single element that satisfies the specified predicate (first operand) followed by the specified regular expression (second operand).</summary>
        public static TGenerex operator +(Predicate<T> one, GenerexWithResultBase<T, TResult, TGenerex, TGenerexMatch> two) { return new Generex<T>(one).then<TGenerex, TGenerexMatch, TResult>((TGenerex) two); }
